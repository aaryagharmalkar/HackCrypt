...
export default function CreditScorePage() {
    ...
    // Function to get score tier based on score value
    const getScoreTier = (score: number) => {
        if (score >= 800) return { tier: "Elite", color: "purple", range: "800-900" };
        if (score >= 700) return { tier: "Excellent", color: "green", range: "700-800" };
        if (score >= 600) return { tier: "Good", color: "blue", range: "600-700" };
        if (score >= 500) return { tier: "Fair", color: "yellow", range: "500-600" };
        if (score >= 400) return { tier: "Poor", color: "orange", range: "400-500" };
        return { tier: "Very Poor", color: "red", range: "300-400" };
    };
        ...
        <div className="space-y-10">
            <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-muted-foreground font-medium mt-1">Understanding your financial trustworthiness and CIBIL score.</p>
                </div>
                    ...
                    <CardContent className="p-10 flex flex-col items-center">
                        <div className="relative w-64 h-64 mb-10">
                            ...
                            <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
                                <span className="text-sm font-bold text-muted-foreground uppercase tracking-widest mb-1">CIBIL</span>
                                <span className="text-6xl font-black text-foreground">{score}</span>
                                <Badge variant={score >= 750 ? "default" : score >= 650 ? "secondary" : "destructive"} className="mt-2 px-4 py-1 font-bold">
                                    {scoreTier.tier}
                                </Badge>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 w-full border-t border-border pt-10">
                            <div className="text-center">
                                <p className="text-xs font-bold text-muted-foreground uppercase mb-1">Payment History</p>
                                <p className="text-sm font-black text-primary uppercase">Excellent</p>
                            </div>
                            <div className="text-center">
                                <p className="text-xs font-bold text-muted-foreground uppercase mb-1">Credit Usage</p>
                                <p className="text-sm font-black text-foreground uppercase">15%</p>
                            </div>
                            <div className="text-center">
                                <p className="text-xs font-bold text-muted-foreground uppercase mb-1">Age of Credit</p>
                                <p className="text-sm font-black text-foreground uppercase">5.2 Years</p>
                            </div>
                            <div className="text-center">
                                <p className="text-xs font-bold text-muted-foreground uppercase mb-1">Recent Inquiries</p>
                                <p className="text-sm font-black text-orange-500 uppercase">High (4)</p>
                            </div>
                        </div>
                    </CardContent>
                </Card>
            ...
            <Card className="border-none shadow-md">
                ...
                <CardContent className="space-y-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="flex gap-4 p-5 rounded-3xl bg-primary/5 border border-primary/20">
            ...
            {/* Roadmap CTA Card */}
            <Card className="border-none shadow-md bg-gradient-to-r from-primary to-secondary text-white">
                <CardContent className="p-8 flex flex-col md:flex-row items-center justify-between gap-8">
                    <div className="flex-1">
                        <h3 className="text-2xl font-bold mb-2">Want to reach {scoreTier.range.includes('800') ? '900' : scoreTier.range.split('-')[1]}+?</h3>
                        <p className="text-white/70 leading-relaxed">
                            Unlock your Personalized Credit Roadmap. Get step-by-step instructions on how to optimize your credit mix and increase your limits safely.
                        </p>
                        <div className="mt-4 flex items-center gap-2">
                            <Badge className="bg-white/20 text-white border-white/30">
                                Current Tier: {scoreTier.tier}
                            </Badge>
                            <Badge className="bg-white/20 text-white border-white/30">
                                Target: {scoreTier.range.includes('800') ? 'Elite' : 
                                        score >= 700 ? 'Elite' :
                                        score >= 600 ? 'Excellent' :
                                        score >= 500 ? 'Good' :
                                        score >= 400 ? 'Fair' : 'Poor'}
